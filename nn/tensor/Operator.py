...
from nn.utils_smc import smc_inner_product_parallel_float
...
class Conv2D(Operator):
    ...
    def __init__(self, kernel_shape=list, input_variable=Variable, name=str, stride=1, padding='SAME'):
        ...
        if self.padding == 'SAME':
            _output_shape = [self.batch_size, ceil(input_variable.shape[1] / stride), ceil(input_variable.shape[2] / stride),
                             self.output_num]
        if self.padding == 'VALID':
            _output_shape = [self.batch_size, ceil((input_variable.shape[1] - self.ksize + 1) / stride),
                             ceil((input_variable.shape[2] - self.ksize + 1) / stride), self.output_num]
    ...
    def _conv(self, input=Variable, output=Variable, weights=np.ndarray, bias=np.ndarray):
        ...
        if self.padding == 'SAME':
            batch_img = np.pad(input.data, (
                (0, 0), (ceil(self.ksize / 2), ceil(self.ksize / 2)), (ceil(self.ksize / 2), ceil(self.ksize / 2)), (0, 0)),
                               'constant', constant_values=0)
        else:
        ...
        # if input.name == 'pool1/out-max-pooling':
        #     print(input.name)
        # else:
        #     print(input.name)
        ...
        if input.name == 'secure-input':
            for i in range(self.batch_size):
                img_i = batch_img[i][np.newaxis, :]
                col_image_i = im2col(img_i, self.ksize, self.stride)
                # dot_res = smc_inner_product_parallel_float(col_weights, col_image_i, 10, 1)
                dot_res = np.dot(col_image_i, (col_weights*100).astype(int))/100
                conv_out[i] = np.reshape(dot_res + bias, output.data[0].shape)
                self.col_image.append(col_image_i)
        else:
            for i in range(self.batch_size):
                img_i = batch_img[i][np.newaxis, :]
                col_image_i = im2col(img_i, self.ksize, self.stride)
                print("DEBUUUUUUGGG-start")
...
class MaxPooling(Operator):
    def __init__(self, ksize=2, input_variable=Variable, name=str, stride=2):
        ...
        self.output_variables = Variable(_output_shape, name='out-max-pooling', scope=name)
        Operator.__init__(self, name, self.input_variables, self.output_variables)
