...
# only for test, set default p, q to save test time
...
def initialize_feip(size, secparam=256):
    if feip == None:
        fe = FEInnerProduct(group, p, q)
        mpk, msk = fe.setup(size, secparam=256)
    else:
        if feip.eta == size:
            return feip, mpk_feip, msk_feip
        else:
            fe = FEInnerProduct(group, p, q)
            mpk, msk = fe.setup(size, secparam=256)
    return fe, mpk, msk
...
def initialize_fefo(secparam=256):
    if fefo is None:
        fe = FEFundamentalOperation(group, p, q)
        mpk, msk = fe.setup(secparam=256)
        return fe, mpk, msk
    else:
        return fefo, mpk_fefo, msk_fefo
