...
class LeNet5:
    ...
    def train_and_eval(self, training_data, training_label, batch_size, epoch, testing_data, testing_label):
        train_loss_hist = list()
        train_total_acc_hist = list()
        train_batch_acc_hist = list()
        test_acc_hist = list()
        ...
        for e in range(epoch):
            for batch_index in range(0, training_data.shape[0], batch_size):
                ...
                train_time_hist.append(stop_time-start_time)

                loss /= batch_size
                batch_acc = float(acc) / float(batch_size)
                training_acc = float(total_acc) / float((batch_index + batch_size) * (e + 1))
                train_loss_hist.append(loss)
                train_batch_acc_hist.append(batch_acc)
                train_total_acc_hist.append(training_acc)

                test_size = 200
                random_idx = random.randint(0, len(testing_data) - test_size)
                test_acc_hist.append(self._eval(
                    testing_data[random_idx: random_idx + test_size],
                    testing_label[random_idx: random_idx + test_size], test_size))

        return train_loss_hist, train_total_acc_hist, train_batch_acc_hist, test_acc_hist, train_time_hist