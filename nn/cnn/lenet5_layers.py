...
class Convolution2D:
    ...
    def __init__(self, inputs_channel, num_filters, kernel_size, padding, stride, learning_rate, name, smc=None):
        # weight size: (F, C, K, K)
        # bias size: (F)
        self.F = num_filters
        self.K = kernel_size
        ...
        if self.smc is not None:
            self.sk = [self.smc.cnn_server_key_request(self.weights[i, :, :, :]) for i in range(0, self.F)]
    ...
    def forward(self, inputs, testing_flag=False):
        # input size: (C, W, H)
        # output size: (N, F ,WW, HH)
        # input shape (1, 28, 28)
        # padding input shape (1, 32, 32)
        # weights shape (6, 1, 5, 5)
        # each f: weights shape (1, 5, 5)
        C = inputs.shape[0]
        ...
        if self.smc is None or testing_flag:
            ...
            expt_feature_maps = np.zeros((self.F, WW, HH))
            for f in range(self.F):
                for w in range(WW):
                    for h in range(HH):
