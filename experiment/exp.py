...
def test_exp_time_one_batch():
    ...
    nn_client = NNEMDClient(n_output=10, mini_batches=total_mini_batches, n_features=X_data.shape[1],
                               smc=s2pc_client, random_seed=520)
    nn_server = NNEMDServer(n_output=10, n_features=X_data.shape[1], hidden_layers=[128, 32],
    ...
    nn_server_enhanced = NNEMDServer(n_output=10, n_features=X_data.shape[1], hidden_layers=[128, 32],
                                        l2=0.1, l1=0.0, epochs=1, eta=0.001, alpha=0.01,
    ...
    for id in setup_parties.keys():
        if x_idx_count == (len(setup_parties) - 1):
            ...
            nn_client_enhanced = NNEMDClient(n_output=10, mini_batches=total_mini_batches, n_features=n_features,
...
def test_exp_diff_networks():
    ...
    logger.info('enhanced secure2pc setting: client encrypting DONE')
    for hidden_layers in hidden_layers_sets:
        nn_server_enhanced = NNEMDServer(n_output=10, n_features=X_data.shape[1], hidden_layers=hidden_layers,
                                            l2=0.1, l1=0.0, epochs=1, eta=0.001, alpha=0.01,
                                            decrease_const=0.001, mini_batches=total_mini_batches, smc=es2pc_server)
...
def test_exp_nn_shallow_acc():
    ...
    nn_client_base = NNEMDClient(n_output=10, mini_batches=total_mini_batches, n_features=X_data.shape[1], random_seed=520)
    nn_server_base = NNEMDServer(n_output=10, n_features=X_data.shape[1], hidden_layers=[64],
    ...
    nn_client_s2pc = NNEMDClient(n_output=10, mini_batches=total_mini_batches, n_features=X_data.shape[1],
                                    smc=s2pc_client, random_seed=520)
    nn_server_s2pc = NNEMDServer(n_output=10, n_features=X_data.shape[1], hidden_layers=[64],
                                    l2=0.1, l1=0.0, epochs=epochs, eta=0.001, alpha=0.001,
...
def test_exp_nn_shallow_acc_simulate():
    ...
    for precision in precision_weight_lst:
        nn_client_base = NNEMDClient(n_output=10, mini_batches=total_mini_batches, n_features=X_data.shape[1], random_seed=520)
        nn_server_base = NNEMDServer(n_output=10, n_features=X_data.shape[1], hidden_layers=[512, 256, 128, 64, 32],
                                        l2=0.1, l1=0.0, epochs=epochs, eta=0.001, alpha=0.001,
...
def test_exp_nn_shallow_acc_cmp_simulate():
    ...
    nn_client_base = NNEMDClient(n_output=10, mini_batches=total_mini_batches, n_features=X_data.shape[1],
                                    random_seed=520)
    nn_server_base = NNEMDServer(n_output=10, n_features=X_data.shape[1], hidden_layers=[512, 256, 128, 64, 32],
                                    l2=0.1, l1=0.0, epochs=epochs, eta=0.001, alpha=0.001,
                                    decrease_const=0.0001, mini_batches=total_mini_batches)
